...
class jeedom_serial():
	...
	def __init__(self,device = '',rate = '',timeout = 9,parity = 'N',stopbits = 1,bytesize = 8):
		self.device = device
		self.rate = rate
	...
	def open(self):
		if self.device:
			logging.debug("Open serial port on device: " + str(self.device)+', rate '+str(self.rate)+', timeout : '+str(self.timeout)+', parity '+str(self.parity)+', stopbits '+str(self.stopbits)+', bytesize '+str(self.bytesize))
		else:
			logging.error("Device name missing.")
			return False
		logging.debug("Open Serialport")
		try:  
			self.port = serial.Serial(self.device,self.rate,timeout=self.timeout,parity=self.parity,stopbits=self.stopbits,bytesize=self.bytesize)
